#!/usr/bin/env python3
"""
Ollama Client for Assistant's Consciousness Substrate
...
class OllamaClient:
    ...
    def _test_connection(self):
        ...
        if self.is_cloud:
            # Cloud doesn't expose a local model list endpoint — skip the check
            print(f"   ✅ Ollama Cloud configured: {self.default_model}")
            return
        ...
        try:
            ...
            if not any(self.default_model in name for name in model_names if name):
                print(f"   ⚠️  Model {self.default_model} not found locally")
    ...
    async def chat_completion(
        ...
        **kwargs
        ...
        # Strip 'ollama:' prefix if present (added by get_default_model())
        ...
        **kwargs
        ...
        try:
            ...
            try:
                ...
                if "tools" in str(te) and "tools" in chat_params:
                    raise OllamaError(
                        "Ollama Python library is too old to support tool calls. "
                        "Run: pip install 'ollama>=0.3.0' and restart the service."
                    ) from te
                raise
