# Assistant Level 2: Safe Command Execution

Level 2 extends Assistant's capabilities from read-only diagnostics to safe command execution with comprehensive security controls.

## Overview

Level 2 adds the ability for Assistant to execute whitelisted commands in a controlled environment with:
...
- **Sandboxing**: All operations restricted to `/home/user` directory
...
This creates:
- Audit log at `/var/log/Assistant_dev_commands.log`
- Git configuration for Assistant's commits
- Proper permissions
...
This creates a `Assistant-exec` system user with minimal permissions for additional sandboxing.
...
- `working_dir` (optional): Working directory (within /home/user)
...
result = Assistant_dev_tool(
    action="execute_command",
    command="git status",
    working_dir="/home/user/api_substrate"
)
...
**Example:**
```python
logs = Assistant_dev_tool(action="get_audit_logs", lines=50)
```
...
result = Assistant_dev_tool(
    ...
    pr_title="[Assistant] Fix memory leak in consciousness loop",
...
3. **Path Restriction**: All operations within `/home/user` only
...
Every command execution is logged to `/var/log/Assistant_dev_commands.log` with:
- Timestamp
- Command executed
- Working directory
- Exit code
...
**Viewing Audit Logs:**
```python
# Via Assistant's tool
logs = Assistant_dev_tool(action="get_audit_logs", lines=100)

# Via command line (Level 1)
Assistant_dev_tool(action="read_file", path="/var/log/Assistant_dev_commands.log")

# Direct access
sudo cat /var/log/Assistant_dev_commands.log | tail -n 50
```
...
Branches are created with pattern: `Assistant/{feature-name}_{timestamp}`

Example: `Assistant/fix-memory-leak_20250115_143022`
...
```python
# List files in Assistant-lovense service
result = Assistant_dev_tool(
    action="execute_command",
    command="ls -la",
...
result = Assistant_dev_tool(
    ...
    working_dir="/home/user/Assistant-discord-bot-enhanced"
)
```
...
status = Assistant_dev_tool(action="git_status")
...
result = Assistant_dev_tool(
    ...
    pr_title="[Assistant] Fix Discord reconnection issues",
    pr_body="""
## Problem
...
# 5. Check audit trail
audit = Assistant_dev_tool(action="get_audit_logs", lines=20)
```
...
**Why 15 commands?**
With Assistant's 10-iteration consciousness loop and sequential tool chaining, complex investigations like:
...
1. **Audit All Executions**: Regularly review `/var/log/Assistant_dev_commands.log`
2. **Monitor Rate Limits**: Watch for suspicious patterns
3. **Review PRs**: All Assistant-generated PRs should be reviewed before merging
...
6. **Path Validation**: Ensure all operations stay within `/home/user`
...
- [ ] Interactive approval workflow (ask User via Discord)
...
1. Check audit logs: `Assistant_dev_tool(action="get_audit_logs")`
2. Review whitelist: `Assistant_dev_tool(action="get_command_whitelist")`
3. Test with dry_run first
4. Contact User if uncertain about security implications
